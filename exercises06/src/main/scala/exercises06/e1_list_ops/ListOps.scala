...
class ListOps[A: Integral](list: List[A]) {
  def filterOdd: List[A]  = list.filter(_ % Integral[A].fromInt(2) != Integral[A].fromInt(0))
  def filterEven: List[A] = list.filter(_ % Integral[A].fromInt(2) == Integral[A].fromInt(0))
}
